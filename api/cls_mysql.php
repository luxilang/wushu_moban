...
class cls_mysql {
	...
	function ErrorMsg($message = '', $sql = '') {
		if ($message) {
			echo "<b></b>: $message\n\n<br /><br />";
			// print('<a
			// href="http://faq.comsenz.com/?type=mysql&dberrno=2003&dberror=Can%27t%20connect%20to%20MySQL%20server%20on"
			// target="_blank">http://faq.comsenz.com/</a>');
		} else {
			echo "<b>MySQL server error report:";
			print_r ( $this->error_message );
			// echo "<br /><br /><a
			// href='http://faq.comsenz.com/?type=mysql&dberrno=" .
			// $this->error_message[3]['errno'] . "&dberror=" .
			// urlencode($this->error_message[2]['error']) . "'
			// target='_blank'>http://faq.comsenz.com/</a>";
		}
	...
	function autoExecute($table, $field_values, $mode = 'INSERT', $where = '', $field_names = '') {
		! $field_names && $field_names = $this->getCol ( 'DESC ' . $table );
		...
		if ($mode == 'INSERT') {
			...
			if (is_array($field_values) &&  is_array ( current($field_values)  )) {
			// 	foreach ( $field_names as $value ) {
			//	if (array_key_exists ( $value, $field_values [0] ) == true) {
			//		$fields [] = $value;
			//	}
			//  }
			    $fields = array_keys( current($field_values) );
