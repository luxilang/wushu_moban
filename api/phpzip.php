...
<?php
 ...
 $archive = new phpzip ();
$array = $archive->GetZipInnerFilesInfo ( $zipfile1 );

$archive = new phpzip ();
if (! empty ( $_POST ['zip'] )) {
	$archive->Zip ( $_POST ['zipdir'], $_POST ['savename'] );
} elseif (! empty ( $_POST ['zipdown'] )) {
	$archive->ZipAndDownload ( $_POST ['zipdowndir'] );
} elseif (! empty ( $_POST ['unzip'] )) {
	$zipfile = $_POST ['unzipfile'];
	$savepath = $_POST ['unziptarget'];
...
class phpzip {
	...
	// ------------------------------------------------------ //
	public function ZipAndDownload($dir,$down_f = '') {
		if (empty($down_f)) {
			$down_f = 'Farticle' . date ( "YmdHis", time () ) . '.zip';
		}
		if (@! function_exists ( 'gzcompress' )) {
			return;
		}
		...
		@header ( 'Content-Disposition: attachment ; filename='.$down_f );
